...
function parentDashboard(){
  ...
  shell(`
    <div class="card">
      ...
      <p><b>${esc(p.nickname||'Family')}</b> • Age ${p.age} • ⭐ ${p.points||0} • ${p.done.length}/30 completed</p>
      <button class="btn" onclick="switchProfile()">Manage profiles</button>
    </div>
    ...
    <div class="card">
      ...
      <div class="setting">
        ...
        <button class="btn" onclick="toggleSetting('large')">${state.settings.large?'On':'Off'}</button>
      </div>
      <div class="setting">
        <span>High contrast</span>
        <button class="btn" onclick="toggleSetting('contrast')">${state.settings.contrast?'On':'Off'}</button>
      </div>
    </div>

    <div class="card">
      <h2>Testing & data</h2>
      <p>Export contains local prototype progress and feedback. Review it before sharing.</p>
      <button class="btn" onclick="feedback()">Leave prototype feedback</button>
      <button class="btn" onclick="exportData()">Export my data</button>
    </div>

    <div class="card">
      <h2>Reset</h2>
      <p>Erase KindStepz prototype data from this browser.</p>
      <button class="btn danger" onclick="resetAll()">Erase local data</button>
    </div>
